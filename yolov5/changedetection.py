...
class ChangeDetection:
    ...
    def send(self, save_dir, image):
        ...
        full_path = str(save_path / '{0}-{1}-{2}-{3}.jpg'.format(today.hour, today.minute, today.second, today.microsecond))

        dst = cv2.resize(image, dsize=(320, 240), interpolation=cv2.INTER_AREA)
        cv2.imwrite(full_path, dst)

        headers = {'Authorization' : 'JMT ' + self.token, 'Accept' : 'application/json'}
